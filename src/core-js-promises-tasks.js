...
function getFirstResolvedPromiseResult(promises) {
  return Promise.allSettled(promises).then((results) => {
    const firstResolvedPromise = results.find(
      (result) => result.status === 'fulfilled'
    );
    if (firstResolvedPromise) {
      return Promise.resolve(firstResolvedPromise.value);
    }
    return Promise.reject(new Error(results[0].reason));
  });
}
...
function queuePromises(promises) {
  return promises.reduce((chain, promise) => {
    return chain.then((value) => {
      return promise.then((result) => value + result);
    });
  }, Promise.resolve(''));
}
